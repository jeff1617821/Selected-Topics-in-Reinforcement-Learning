...
class DQNBaseAgent(ABC):
	...
	def update_behavior_network(self):
		...
		### TODO ###
		# calculate the loss and update the behavior networ		
	...
	def train(self):
		episode_idx = 0
		TrainEpisode_Reward_x = []
		TrainEpisode_Reward_y = []
		EvaluateEpisode_Reward_x = []
		EvaluateEpisode_Reward_y = []
		while self.total_time_step <= self.training_steps:
			...
			while True:
				...
				if terminate or truncate:
					...
					TrainEpisode_Reward_x.append(self.total_time_step)
					TrainEpisode_Reward_y.append(episode_reward)
					break
			...
			if episode_idx % self.eval_interval == 0:
				...
				EvaluateEpisode_Reward_x.append(self.total_time_step)
				EvaluateEpisode_Reward_y.append(avg_score)
				self.save(os.path.join(self.writer.log_dir, f"model_{self.total_time_step}_{int(avg_score)}.pth"))
				self.writer.add_scalar('Evaluate/Episode Reward', avg_score, self.total_time_step)
		TrainEpisode_Reward_x = np.asarray(TrainEpisode_Reward_x)
		TrainEpisode_Reward_y = np.asarray(TrainEpisode_Reward_y)
		TrainEpisode_Reward_x.tofile('TrainEpisode_Reward_x.csv', sep = ',')
		TrainEpisode_Reward_y.tofile('TrainEpisode_Reward_y.csv', sep = ',')
		EvaluateEpisode_Reward_x = np.asarray(EvaluateEpisode_Reward_x)
		EvaluateEpisode_Reward_y = np.asarray(EvaluateEpisode_Reward_y)
		EvaluateEpisode_Reward_x.tofile('EvaluateEpisode_Reward_x.csv', sep = ',')
		EvaluateEpisode_Reward_y.tofile('EvaluateEpisode_Reward_y.csv', sep = ',')
